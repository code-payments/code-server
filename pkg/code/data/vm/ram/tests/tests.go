...
func testHappyPath(t *testing.T, s ram.Store) {
	t.Run("testHappyPath", func(t *testing.T) {
		...
		address, freedIndex1, err := s.ReserveMemory(ctx, "vm1", cvm.VirtualAccountTypeTimelock)
		require.NoError(t, err)
		assert.Equal(t, "address1", address)
		assert.True(t, freedIndex1 == 42 || freedIndex1 == 66)

		address, freedIndex2, err := s.ReserveMemory(ctx, "vm1", cvm.VirtualAccountTypeTimelock)
		require.NoError(t, err)
		assert.Equal(t, "address1", address)
