...
func TestSendMessage_RequestToReceiveBill_KinValue_Validation(t *testing.T) {
	...
	sendMessageCall.assertInvalidMessageError(t, "core mint exchange rate must be 1")
	env.server1.assertNoMessages(t, rendezvousKey)
	env.server1.assertRequestRecordNotSaved(t, rendezvousKey)
	...
	sendMessageCall.assertInvalidMessageError(t, "usdc currency has a minimum amount of 0.05")
	env.server1.assertNoMessages(t, rendezvousKey)
	env.server1.assertRequestRecordNotSaved(t, rendezvousKey)
	...
	sendMessageCall.assertInvalidMessageError(t, "usdc currency has a maximum amount of 5.00")
	env.server1.assertNoMessages(t, rendezvousKey)
	env.server1.assertRequestRecordNotSaved(t, rendezvousKey)
	...
	sendMessageCall.assertInvalidMessageError(t, "exact exchange data is reserved for core mint only")
	env.server1.assertNoMessages(t, rendezvousKey)
	env.server1.assertRequestRecordNotSaved(t, rendezvousKey)
