...
func LocalSimulation(ctx context.Context, data code_data.Provider, actions []*transactionpb.Action) (*LocalSimulationResult, error) {
	...
	for _, action := range actions {
		...
		case *transactionpb.Action_OpenAccount:
			mint, err = getBackwardsCompatMint(typedAction.OpenAccount.Mint)
			if err != nil {
				return nil, err
			}
		...
		case *transactionpb.Action_NoPrivacyTransfer:
			mint, err = getBackwardsCompatMint(typedAction.NoPrivacyTransfer.Mint)
			if err != nil {
				return nil, err
			}
		...
		case *transactionpb.Action_FeePayment:
			mint, err = getBackwardsCompatMint(typedAction.FeePayment.Mint)
			if err != nil {
				return nil, err
			}
		...
		case *transactionpb.Action_NoPrivacyWithdraw:
			mint, err = getBackwardsCompatMint(typedAction.NoPrivacyWithdraw.Mint)
			if err != nil {
				return nil, err
			}
