...
func (s *transactionServer) Swap(streamer transactionpb.Transaction_SwapServer) error {
	...
	vmDepositTokenAddress, err := token.GetAssociatedAccount(vmDepositPda, common.CoreMintAccount.PublicKey().ToBytes())
	if err != nil {
	...
	quote, err := s.jupiterClient.GetQuote(
		ctx,
		usdc.Mint,
		common.CoreMintAccount.PublicKey().ToBase58(),
		amountToSwap,
...
func (s *transactionServer) validateSwap(
	...
	otherAmount := float64(quote.GetEstimatedSwapAmount()) / float64(common.CoreMintQuarksPerUnit)
	swapRate := usdcAmount / otherAmount
