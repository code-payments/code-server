...
func TestSendChatMessage_HappyPath(t *testing.T) {
	...
	chatId := chat.GetChatId(chatTitle, receiver.PublicKey().ToBase58(), true)
...
func (e *testEnv) assertChatRecordSaved(t *testing.T, chatTitle string, receiver *common.Account, isVerified bool) {
	chatId := chat.GetChatId(chatTitle, receiver.PublicKey().ToBase58(), isVerified)

	chatRecord, err := e.data.GetChatById(e.ctx, chatId)
	require.NoError(t, err)
