...
const (
	DepositFromPdaInstructionArgsSize = (2 + // account_index
		8 + //amount
		1) // bump
)

type DepositFromPdaInstructionArgs struct {
	AccountIndex uint16
	Amount       uint64
	Bump         uint8
}

type DepositFromPdaInstructionAccounts struct {
...
func NewDepositFromPdaInstruction(
	accounts *DepositFromPdaInstructionAccounts,
	args *DepositFromPdaInstructionArgs,
) solana.Instruction {
	var offset int

	// Serialize instruction arguments
	data := make([]byte, 1+DepositFromPdaInstructionArgsSize)

	putCodeInstruction(data, CodeInstructionDepositFromPda, &offset)
