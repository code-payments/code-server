...
const (
	TimelockWithdrawInternalVirtrualInstructionDataSize = SignatureSize // signature
)

type TimelockWithdrawInternalVirtualInstructionArgs struct {
	TimelockBump uint8
	Signature    Signature
}

type TimelockWithdrawInternalVirtualInstructionAccounts struct {
...
func NewTimelockWithdrawInternalVirtualInstructionCtor(
	accounts *TimelockWithdrawInternalVirtualInstructionAccounts,
	args *TimelockWithdrawInternalVirtualInstructionArgs,
) VirtualInstructionCtor {
	return func() (Opcode, []solana.Instruction, []byte) {
		var offset int
		data := make([]byte, TimelockWithdrawInternalVirtrualInstructionDataSize)
		...
		return OpcodeTimelockWithdrawToInternal, ixns, data
	}
}
