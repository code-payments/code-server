...
const (
	SellAndDepositIntoVmInstructionArgsSize = (8 + // in_amount
		8 + // min_out_amount
		2) // account_index
)
...
func NewSellAndDepositIntoVmInstruction(
	accounts *SellAndDepositIntoVmInstructionAccounts,
	args *SellAndDepositIntoVmInstructionArgs,
) solana.Instruction {
	var offset int

	// Serialize instruction arguments
	data := make([]byte, 1+SellAndDepositIntoVmInstructionArgsSize)

	putInstructionType(data, InstructionTypeSellAndDepositIntoVm, &offset)
